...
\begin{question}
  ...
  \item Our friend \textit{Geometry Giorgio} claims it is obvious that of all the
    triangles with a fixed area and fixed height, an \textbf{isosceles triangle}
  ...
  \item Imagine a triangle with two points that are fixed, and a third
    ...
    fixed. \textbf{What is the locus of points determined by the third
...
\begin{question}
  In 1973, \textit{FedEx} was a small company. They had a very clever idea:
  ...
  check out \link[what \textit{FedEx} has to say about
    this]{https://www.fedex.com/en-us/about/policy/aviation/why-memphis.html}.
  The idea was very successful, and is now standard practice in the
  industry.
    ...
    \textbf{This obtuse triangle:}
      ...
      \raisebox{-.4\height}{
        ...
        \textbf{This right triangle:}
          ...
          \newpage
        \item Notice where the circumcenters were in relation to the
          triangles in each case above. Generalize your work above,
          and make THREE statements of the form:
          \begin{quote}
            If a triangle is (\textit{acute/right/obtuse}) then the
            circumcenter for the triangle is (\textit{give some sort of
            location}).
          \end{quote}
  \end{enumerate}
  \begin{freeResponse}
...
\item PROPOSE an alternate method for detecting how ``circular'' the
  cross section of the solid rocket booster is by measuring ``chords''
  of the cross section. \textbf{Try your method on the THREE pictures
  ...
  \begin{freeResponse}
    ...
    \item Geometry Georigio's technique sounds good. Let's put it to the test!
      ...
      \item Answers may vary. A fine solution is to simply measure the
        ...
        cross-section. One cannot check by simply drawing a circle. 
        ...
        A nice solution is to measure the diameter, then make a
        ``right triangle'' by connecting any other points. We could
        then use the Pythagorean theorem to identify side lengths. 
        
        
        These methods are better than \textit{Geometry
