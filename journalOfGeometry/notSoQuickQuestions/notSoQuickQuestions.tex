...
\begin{listOutcomes}
\item follow algoritm
\item translate school math real world math
\item 
\end{listOutcomes}
    ...
    \item If both slopes are the same, then our picture looks like
      ...
      \begin{center}
        \begin{tikzpicture}[geometryDiagrams,scale=.2]
          ...
          \draw[decoration={brace,raise=.2cm,mirror},decorate,thin] (X)--(B);
          \node[anchor=north east] at (-5,-6.5) {$\sqrt{12^2+12^2}$};
        \end{tikzpicture}
      \end{center}
...
\begin{question}
  ...
  \begin{enumerate}
    ...
    \item If all the slopes of a roof are the same, there is an even
      easier method of finding the area of the roof using
      SCALING. Discover this method.
  \end{enumerate}
\end{question}
