...
\begin{document}
\begin{abstract}
  We draw curves with lines and strings.
\end{abstract}
\maketitle
...
\begin{question}
  Carefully draw a $15$-line envelope of tangents, preferably by hand. Use a ruler or the folding method to find a midpoint. Make it big, take up the whole page!
  \begin{freeResponse}
    NOT DONE
  \end{freeResponse}
\end{question}
...
\begin{tikzpicture}[geometryDiagrams,scale=.2]
 \draw (-19, -19)--(-19,10)--(10,10);
 \node at (0,10) {$|$};
 \node at (-19,0) {$--$};
  \node at (-10,11) {$a$};
 \node at (-20,5) {$b$};
         \draw[dashed,thin] (0,0) ellipse (19 and 10);
\end{tikzpicture}
\end{center}
...
\[
\includegraphics[width=.3\textwidth]{bezier.pdf}
\]
This polygonal path in turn gives an envelope of tangents, which then gives
the desired curve:
\[
\includegraphics[width=.5\textwidth]{bezierMesh.pdf}
\]
  ...
  \item One way to solve this problem is to think of the curve above
    ...
    \item Reproduce the drawing above for the archway including the
     correct measurements for $c$, assuming that $f_1$ and $f_2$ are
      the foci of an ellipse that draws the desired curve. Show all
      work and explain your reasoning.
         \item Explain how someone could physically draw the required
      ellipse using a \textbf{pencil}, \textbf{two nails}, and a
      \textbf{string}.
    \end{enumerate}
...
%    modifying the block
%    \raisebox{-.4\height}{\includegraphics{circleBlank.png}} and using
...
%    \raisebox{-.4\height}{\includegraphics{board.png}} to draw a scale copy of the board for
%    you.
% 
  ...
  \item Considering the generic ellipse above, we see that the
    ...
    \begin{tikzpicture}[geometryDiagrams,scale=.8]
      ...
      \node[below] at (1.85in,-.1in) {$5''$};
        ...
        \end{tikzpicture}
    \end{center}
  \item From our work above, we know that $a=19$ and $b=10$. So we can make an ellipse script:
    \begin{center}
      \includegraphics{ellipseSCRIPT.png}
    \end{center}
    Putting this together with the  \raisebox{-.4\height}{\includegraphics{board.png}} block, we have
    \begin{center}
      \includegraphics[width=.3\textwidth]{laserSCRIPT.png}   \qquad \fbox{\includegraphics[width=.5\textwidth]{laserSTAGE.png}}
    \end{center}
  \item Finally, here is my picture:
    \begin{center}
      \includegraphics{envARCHWAY.png}
    \end{center}
    \end{enumerate}
  \end{freeResponse}
\end{question}
