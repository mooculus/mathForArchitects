...
\begin{question}    
We can extend the definition of sine to any triangle if we
      interpret it as follows: 
      Given any triangle, like the two above, 
      find a height $h$ perpendicular to side $b$ (or side $c$). Then use the right triangle to $\sin(\theta)$. 

\begin{enumerate}
 \item Use similar triangles to explain why Explain why
      \[
      \frac{h}{c} = \frac{h'}{b}
      \]
	...
	\draw (0,0)--(5,2) node [midway,yshift=.2cm] 
          {$c$};
          
          \draw [decorate,decoration={brace,amplitude=10pt,mirror}](0,0)--(7,0) node [midway,yshift=-1cm] 
          {$b$};
          \draw (7,0)--(5,2) node [midway,yshift=.2cm] 
          {$a$};
	...
	\draw (0,0)--(5,2) node [midway,yshift=.2cm] 
          ...
          \draw (0,0)--(7,0) node [midway,yshift=-1cm] 
          {$b$};
          \draw (7,0)--(5,2) node [midway,yshift=.2cm] 
          {$a$};
          \tkzMarkAngle[size=1.5cm,thin,mark=](C,A,B)
     ...
     \item       
       Explain why in this definition of sine, {it doesn't matter which
        height we choose, the value of $\sin(\theta)$ will be the same.}  Find $\sin(\theta)$ in the diagram above.
        \end{enumerate}
\end{question}
...
\begin{question}
  ...
  \begin{quote}
    ``Sine and cosine as encoding information about similar right
  triangles.''
  \end{quote}
\end{question}
